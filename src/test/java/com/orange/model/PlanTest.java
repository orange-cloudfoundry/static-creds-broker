...
public class PlanTest {
    ...
    public void plan_metadata_is_set_using_json() throws Exception {
        ...
        assertThat(plan.getMetadata().getBullets()).containsOnly("20 GB of messages","20 connections");
        assertThat(plan.getMetadata().getDisplayName()).isEqualTo("Big Bunny");
    ...
    public void plan_credentials_may_be_set_using_json() throws Exception {
        ...
        assertThat(plan.getCredentialsJson()).hasSize(4).includes(entry("username", "admin"), entry("password", "pa55woRD"),entry("uri", "mysql://USERNAME:PASSWORD@HOSTNAME:PORT/NAME"),entry("hosname", "us-cdbr-east-03.cleardb.com"));
    }
    ...
    public void plan_is_free_by_default(){
        ...
        assertThat(defaultPlan.getFree()).isTrue();
    }

    @Test
    public void plan_has_a_default_name(){
        Plan defaultPlan = new Plan();
        assertThat(defaultPlan.getName()).isNotEmpty();
        assertThat(defaultPlan.getName()).isEqualTo("default");
    }

    @Test
    public void plan_has_a_default_description(){
        Plan defaultPlan = new Plan();
        assertThat(defaultPlan.getDescription()).isNotEmpty();
    }
